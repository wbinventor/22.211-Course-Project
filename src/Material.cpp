...
/**
 * Material constructor sets default values for some material properties
 */
Material::Material() {
	...
	/* By default this material has no cross-sections */
	_num_capture_xs = 0;
	_num_scatter_xs = 0;
	_num_elastic_xs = 0;
	_num_inelastic_xs = 0;
...
/**
 * Material destructor deletes array of cross section values that
 * have been assigned to this material
 */
Material::~Material() {
 ...
 */
char* Material::getIsotopeType() const {
	return _isotope;
}


/**
 * Returns the atomic number of this material
 * @return the atomic number
 */
int Material::getA() const {
    return _A;
}


/**
 * Returns the alpha ((A-1)/(A+1))^2 values for this material
 * @return alpha
 */
...
/**
 * Returns the number density for this material
...
/**
 * Return the temperature (Kelvin) for this material
 * @return the temperature of this material
 */
float Material::getTemperature() const {
	return _T;
}


/**
 * Return the average value of the cosine of theta for this material
 * in a scattering collision
 * @return the average for mu
 */
float Material::getMuAverage() const {
	return _mu_avg;
}
 ...
 */
float Material::getCaptureXS(float energy) const{
 ...
 */
float Material::getOneGroupCaptureXS() const {
	if (_num_capture_xs == 0)
		return 0.0;
 ...
 * @return the scattering cross-section (barns)
 */
float Material::getScatterXS(float energy) const {
	if (_num_scatter_xs == 0 && _num_inelastic_xs == 0 && _num_elastic_xs == 0)
		return 0.0;

	/* If this material has both inelastic and elastic scattering */
	else if (_num_scatter_xs == 0 && _num_inelastic_xs != 0 &&
												_num_elastic_xs != 0)
		return (getInelasticXS(energy) + getElasticXS(energy));

	/* If this material only has elastic scattering */
	else if (_num_scatter_xs == 0 && _num_inelastic_xs == 0 &&
												_num_elastic_xs != 0)
		return getElasticXS(energy);

	/* If this material only has inelastic scattering */
	else if (_num_scatter_xs == 0 && _num_inelastic_xs != 0 &&
												_num_elastic_xs == 0)
 ...
 * @return the scattering cross-section (barns)
 */
float Material::getOneGroupScatterXS() const {
...
/**
 * Returns the type of angular scattering distribution for this material
 * (ISOTROPIC_CM or ISOTROPIC_LAB)
 * @return the type of angular scattering distribution
 */
scatterAngleType Material::getScatterAngleType() const {

	if (_num_scatter_xs == 0 && _num_inelastic_xs == 0 && _num_elastic_xs == 0)
		log_printf(ERROR, "Cannot return a scatter angle type"
				"for material %s since it has not been set", _isotope);
 ...
 */
float Material::getInelasticXS(float energy) const {
 ...
 */
float Material::getOneGroupInelasticXS() const {
 ...
 * this material (ISOTROPIC_CM or ISOTROPIC_LAB)
 * @return the type of angular scattering distribution
 */
scatterAngleType Material::getInelasticAngleType() const {
	if (_num_inelastic_xs == 0)
 ...
 */
float Material::getElasticXS(float energy) const {
 ...
 */
float Material::getOneGroupElasticXS() const {
 ...
 * @return the type of angular scattering distribution
 */
scatterAngleType Material::getElasticAngleType() const {
 ...
 */
float Material::getFissionXS(float energy) const{
 ...
 * @return the fission cross-section (barns)
 */
float Material::getOneGroupFissionXS() const {
	if (_num_fission_xs == 0)
		return 0.0;
 ...
 */
float Material::getAbsorbXS(float energy) const {
	if (_num_fission_xs == 0 && _num_capture_xs == 0)
		return 0.0;
 ...
 * @return the absorption cross-section (barns)
 */
float Material::getOneGroupAbsorbXS() const {
 ...
 * energy based on linear interpolation
 * @param energy the energy (eV) of interest
 * @return the total scattering cross-section (barns)
 */
float Material::getTotalXS(float energy) const {
	 ...
	 * add them to a total xs */
		...
		std::map<collisionType, float(Material::*)(float)
 ...
 * @return the elastic cross-section (barns)
 */
float Material::getOneGroupTotalXS() const {
...
/**
 * Returns the transport cross-section for this material at a particular energy
 * @param energy the energy (eV) of interest
 * @return the transport cross-section (barns)
 */
float Material::getTransportXS(float energy) const {
 ...
 */
float Material::getInelasticScatterEnergy(float energy) {
 ...
 * for this material have been initialized, and false otherwise
 * @return boolean if the thermal scattering distributions exist
 */
bool Material::usesThermalScattering() {
 ...
 */
void Material::setIsotopeType(char* isotope) {
	_isotope = isotope;
}
 ...
 */
void Material::setTemperature(float T) {
	_T = T;
}


/**
 * Load a cross-section from an ASCII file into this material
 ...
 */
void Material::loadXS(char* filename, collisionType type, char* delimiter) {
	...
	/* Set this material's appropriate cross-section using the data
	 * structures */
	if (type == CAPTURE)
...
/**
 * Set the capture cross-section for this material
 ...
 */
void Material::setCaptureXS(float* capture_xs, float* capture_xs_energies,
													int num_capture_xs) {
    _capture_xs = capture_xs;
    _capture_xs_energies = capture_xs_energies;
    _num_capture_xs = num_capture_xs;
    float (Material::*func)(float) const;
    func = &Material::getCaptureXS;
    _xs_handles.insert(std::pair<collisionType, float(Material::*)(float)
    											const>(CAPTURE, func));
}


/**
 * Set the one group capture cross-section for this material
 * @param capture_xs the one group capture cross-section (barns)
 */
void Material::setOneGroupCaptureXS(float capture_xs) {
...
/**
 * Set the scattering cross-section for this material
 ...
 */
void Material::setScatterXS(float* scatter_xs, float* scatter_xs_energies,
...
/**
 * Set the one group scattering cross-section for this material
 * @param scatter_xs the one group scattering cross-section (barns)
 */
void Material::setOneGroupScatterXS(float scatter_xs, scatterAngleType type) {
...
/**
 * Set the type of angular scattering distribution for this material
 * @param type the angular scattering distribution type
 */
void Material::setScatterAngleType(scatterAngleType type) {
	_scatter_angle = type;
}


/**
 * Set the inelastic cross-section for this material
 ...
 * @param type the type of angular scattering distribution
 */
void Material::setInelasticXS(float* inelastic_xs,
...
/**
 * Set the one group inelastic scattering cross-section for this material
 * @param inelastic_xs the one group inelastic scattering cross-section (barns)
 */
void Material::setOneGroupInelasticXS(float inelastic_xs,
													scatterAngleType type) {
...
/**
 * Set the type of angular inelastic scattering distribution for this material
 * @param type the angular inelastic scattering distribution type
 */
void Material::setInelasticAngleType(scatterAngleType type) {
	_inelastic_angle = type;
}


/**
 * Set the elastic cross-section for this material
 ...
 * @param type the type of angular scattering distribution
 */
void Material::setElasticXS(float* elastic_xs, float* elastic_xs_energies,
...
/**
 * Set the one group elastic scattering cross-section for this material
 * @param elastic_xs the one group elastic scattering cross-section (barns)
 */
void Material::setOneGroupElasticXS(float elastic_xs, scatterAngleType type) {
...
/**
 * Set the type of angular elastic scattering distribution for this material
 * @param type the angular elastic scattering distribution type
 */
void Material::setElasticAngleType(scatterAngleType type) {
	_elastic_angle = type;
}


/**
 * Set the fission cross-section for this material
 ...
 */
void Material::setFissionXS(float* fission_xs, float* fission_xs_energies,
													int num_fission_xs) {
    _fission_xs = fission_xs;
    _fission_xs_energies = fission_xs_energies;
    _num_fission_xs = num_fission_xs;
    float (Material::*func)(float) const;
    func = &Material::getFissionXS;
    _xs_handles.insert(std::pair<collisionType, float(Material::*)(float)
    											const>(FISSION, func));
}
...
/**
 * Set the one group fission cross-section for this material
 * @param fission_xs the one group fission cross-section (barns)
 */
void Material::setOneGroupFissionXS(float fission_xs) {
...
/**
 * Set the total cross-section for this material
 ...
 */
void Material::setTotalXS(float* total_xs, float* total_xs_energies,
													int num_total_xs) {
...
/**
 * Set the one total elastic scattering cross-section for this material
 * @param total_xs the one group total cross-section (barns)
 */
void Material::setOneGroupTotalXS(float total_xs) {
...
/**
 * Add an inelastic scattering cdf to this material which applies to incoming
 ...
 */
void Material::addInelasticScatterCDF(float lower_bound, float upper_bound,
 ...
 */
collisionType Material::getCollisionType(float energy) {
 ...
 * @return the collision type (CAPTURE, SCATTER, INELASTIC, ELASTIC, TOTAL)
 */
collisionType Material::getOneGroupCollisionType() {
 ...
 */
float Material::getThermalScatteringEnergy(float energy) {
	...
	if (_num_thermal_cdfs == 0)
		log_printf(ERROR, "Unable to sample the thermal scattering CDFs for"
				" material %s because they have not yet been initialized",
																_isotope);
...
Material* Material::clone() {
	...
	new_clone->setIsotopeType(_isotope);
	new_clone->setA(_A);
	new_clone->setN(_N);
	new_clone->setTemperature(_T);

	/* If the given material has a one group capture xs */
	if (_num_capture_xs == 1)
		new_clone->setOneGroupCaptureXS(_capture_xs[0]);


	/* If the given material has an capture xs */
	if (_num_capture_xs > 1) {
	...
	/* If the given material has a one group scatter xs */
	if (_num_scatter_xs == 1)
		new_clone->setOneGroupScatterXS(_scatter_xs[0], _scatter_angle);


	/* If the given material has a scatter xs */
	if (_num_scatter_xs > 1) {
	...
	/* If the given material has an inelastic scatter xs */
	...
	/* If the given material has an elastic scatter xs */
	...
	/* If the given material has a one group fission xs */
	if (_num_fission_xs == 1)
		new_clone->setOneGroupFissionXS(_fission_xs[0]);


	/* If the given material has a fission xs */
	if (_num_fission_xs > 1) {
	...
	/* If the given material has an explicitly defined total scatter xs */
	if (_num_total_xs == 1)
	...
	/* Deep copy the material's inelastic scatter cdfs */
	...
	/* Initialize the material's thermal scattering CDFs if they have been
	...
	/* Return a pointer to the cloned Material class */
	return new_clone;
}
 ...
 */
void Material::initializeThermalScattering(float start_energy,
 ...
 */
float Material::thermalScatteringProb(float E_prime_to_E, int dist_index) {
 ...
 * of cross-section types for this material. The cross-section types are
 ...
 */
void Material::plotXS(float start_energy, float end_energy, int num_energies,
													collisionType types, ...) {
	...
	title << "set title \"" << _isotope;
	title << " Microscopic Cross-sections\"";
	filename << _isotope << "_micro_xs";
...
/**
 * Plot the thermal scattering distributions for this material if they have
 * been initialized
 */
void Material::plotThermalScatteringDistributions() {
	...
	std::stringstream title;
	std::stringstream filename;
	title << "set title \"" << _isotope << " Thermal Scattering Dist\"";
	filename << _isotope << "_therm_dist";
...
/**
 * Plot the thermal scattering CDFs for this material type
 */
void Material::plotThermalScatteringCDFs() {
	...
	std::stringstream title;
	std::stringstream filename;
	title << "set title \"" << _isotope << " Thermal Scattering CDFs\"";
	filename << _isotope << "_therm_cdfs";
...
/**
 * This method plots the material's thermal scattering CDFs sampled at a
 * certain energy
 * @param energy the energy (eV) to sample
 * @param num_samples the number of samples to plot
 */
void Material::plotSampledThermalScatteringEnergies(float energy,
												int num_samples) {
	...
	std::stringstream title;
	std::stringstream filename;
	title << "set title \"" << _isotope << " Sampled Thermal Scattering "
			<< "Eprimes (for E = " << energy << ")\"";
	filename << _isotope << "_" << energy << "eV_therm_cdfs";
